...
- `@/lib/realtime/manager` — ChannelBuilder ChannelHandle ChannelStatusListener ChannelSubscribeStatus TEARDOWN_GRACE_MS __channelRefCount __resetRealtimeManager acquireChannel isRealtimeAvailable  
- `@/lib/realtime/subscribe` — BranchBroadcast BranchPostgresEvent BranchSubscriptionHandlers DownReason OrderBroadcast OrderSubscriptionHandlers SubscribeLifecycle branchBroadcastSchema orderBroadcastSchema subscribeToBranch subscribeToOrder  
...
- `@/lib/auth/guards` — CAPABILITY_ROLES Capability hasCapability hasRole isPathReachable landingPathFor loginPathFor requireBranchAccess requireCapability requirePlatformAdmin requireRole requireStaffContext safeNextPath  
- `@/lib/auth/session` — BRANCH_COOKIE StaffBranch StaffContext StaffMembership StaffRestaurant canAccessBranch getSession getStaffContext loadStaffContext  
- `@/lib/services/branch-service` — BranchAdminView createBranch getBranch listBranches previewServiceFee setAcceptingOrders setBranchActive updateBranch  
- `@/lib/services/dashboard-service` — DashboardOptions getDashboardStats  
- `@/lib/services/menu-service` — CategoryAdminView MenuItemAdminView MenuItemFilters createCategory createMenuItem deleteCategory deleteMenuItem getMenuItem listCategories listMenuItemOptions listMenuItems reorder setCategoryActive setItemAvailability updateCategory updateMenuItem  
- `@/lib/services/order-service` — OrderFilters OrderListPage advanceOrderStatus cancelOrder getOrder listKitchenTickets listOrders updateOrderStatus voidLine  
- `@/lib/services/session` — ACTIVE_BRANCH_COOKIE BranchScope Capability getActiveBranchId getBranchScope getStaffSession requireCapability requirePlatformAdmin requireStaffSession  
- `@/lib/services/settings-service` — SettingsView getSettings updateSettings  
- `@/lib/services/staff-service` — StaffAdminView deactivateStaff getStaffMember inviteStaff listStaff updateStaff  
- `@/lib/services/table-service` — TableAdminView TableDetailView createTable deactivateTable deleteTable getTable listTables qrPngDataUrl qrTargetUrl rotateToken setTableActive updateTable  
- `@/lib/services/waiter-service` — acknowledgeCall getWaiterCall listWaiterCalls resolveCall updateWaiterCall  
- `@/lib/demo/demo-mode` — DEMO_BADGE_KEY DEMO_BANNER_KEY DEMO_BODY_KEY DEMO_NOTICE DEMO_TOKENS DemoPlaceOrderArgs __resetDemoStore demoReadOnly demoRepository demoStaffSession  export { DEMO_TOKEN }
- `@/lib/demo/fixture` — DEMO_IDS DEMO_TOKEN DemoFixtures FIXTURES FixtureBranch FixtureCategory FixtureMenuItem FixtureMenuItemOption FixtureOrder FixtureOrderItem FixturePromotion FixtureRestaurant FixtureStaff FixtureTable FixtureWaiterCall  
- `@/lib/mappers/menu-mapper` — MenuItemViewOptions MenuTreeOptions mapPublicTableContext toMenuCategoryView toMenuItemView toMenuOptionGroups toMenuTree toPromotionView toTableContext  
- `@/lib/mappers/order-mapper` — KitchenTicketOptions OrderHistoryJoin OrderItemRowWithOptions OrderRowWithRelations OrderTableJoin OrderViewOptions toKitchenTicket toOrderLineView toOrderView  
- `@/lib/mappers/waiter-mapper` — WaiterCallOrderJoin WaiterCallRowWithRelations WaiterCallStaffJoin WaiterCallTableJoin WaiterCallViewOptions byUrgency toWaiterCallView  
